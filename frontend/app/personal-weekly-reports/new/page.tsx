...
// const taskStatuses: TaskStatus[] = ["Not Started", "In Progress", "Completed", "Blocked"];
...
//     const [hours, setHours] = useState({ development: "", testing: "", meetings: "", documentation: "" });
//     const [notes, setNotes] = useState("");

//     const [saving, setSaving] = useState(false);
//     const [error, setError] = useState("");
//     const [hoursOpen, setHoursOpen] = useState(false);
//     const [fieldErrors, setFieldErrors] = useState<{ weekStart?: string; project?: string; tasks?: string }>({});
//     const [taskErrors, setTaskErrors] = useState<Record<number, { task_name?: string; planned_percentage?: string; actual_percentage?: string; time_planned_hours?: string; time_spent_hours?: string }>>({});
//     const [hoursErrors, setHoursErrors] = useState<Partial<Record<'development' | 'testing' | 'meetings' | 'documentation', string>>>({});

//     useEffect(() => {
//         if (fieldErrors.tasks && tasks.some((t) => t.task_name && t.task_name.trim())) {
...
//     const addBlocker = () => setBlockers((prev) => [...prev, { text: "", is_key: false }]);
//     const removeBlocker = (index: number) => setBlockers((prev) => prev.filter((_, i) => i !== index));

//     const addAchievement = () => setAchievements((prev) => [...prev, { text: "", is_key: false }]);
//     const removeAchievement = (index: number) => setAchievements((prev) => prev.filter((_, i) => i !== index));

//     const handleSave = async (status: Extract<ReportStatus, "Draft" | "Submitted">) => {
//         setError("");
//         setFieldErrors({});
...
//             if (!weekStart) {
//                 nextFieldErrors.weekStart = "Week start date is required.";
//             }
//             if (!project || !project.trim()) {
//                 nextFieldErrors.project = "Project or category tag is required.";
//             }
//             if (tasks.length === 0 || !tasks.some((t) => t.task_name && t.task_name.trim())) {
//                 nextFieldErrors.tasks = "Add at least one task with a task name before submitting.";
//             }
...
//             }
//             // per-task validation
//             const tErrors: typeof taskErrors = {};
//             tasks.forEach((t, idx) => {
//                 if (!t.task_name || !t.task_name.trim()) {
//                     tErrors[idx] = tErrors[idx] || {};
//                     tErrors[idx].task_name = "Task name is required.";
//                 }
//                 if (t.planned_percentage !== undefined && (Number.isNaN(t.planned_percentage) || t.planned_percentage < 0 || t.planned_percentage > 100)) {
//                     tErrors[idx] = tErrors[idx] || {};
//                     tErrors[idx].planned_percentage = "Enter a percent between 0 and 100.";
//                 }
//                 if (t.actual_percentage !== undefined && (Number.isNaN(t.actual_percentage) || t.actual_percentage < 0 || t.actual_percentage > 100)) {
//                     tErrors[idx] = tErrors[idx] || {};
//                     tErrors[idx].actual_percentage = "Enter a percent between 0 and 100.";
//                 }
//                 if (t.time_planned_hours !== undefined && (Number.isNaN(t.time_planned_hours) || t.time_planned_hours < 0)) {
//                     tErrors[idx] = tErrors[idx] || {};
//                     tErrors[idx].time_planned_hours = "Enter a non-negative number.";
//                 }
//                 if (t.time_spent_hours !== undefined && (Number.isNaN(t.time_spent_hours) || t.time_spent_hours < 0)) {
//                     tErrors[idx] = tErrors[idx] || {};
//                     tErrors[idx].time_spent_hours = "Enter a non-negative number.";
//                 }
//             });
//             setTaskErrors(tErrors);
//             // hours breakdown validation
//             const hErrors: typeof hoursErrors = {};
//             Object.entries(hours).forEach(([k, v]) => {
//                 if (v !== "" && Number.isNaN(Number(v))) {
//                     (hErrors as any)[k] = "Enter a valid number.";
//                 } else if (v !== "" && Number(v) < 0) {
//                     (hErrors as any)[k] = "Enter a non-negative number.";
//                 }
//             });
//             setHoursErrors(hErrors);

//             if (Object.keys(tErrors).length > 0 || Object.keys(hErrors).length > 0) {
//                 setError("Please fix the highlighted fields before submitting.");
//                 return;
//             }
//         }
...
//             if (response.success) {
//                 toast.success(status === "Draft" ? "Report saved as draft." : "Report submitted for review.");
//                 router.push("/personal-weekly-reports");
//                 return;
//             }
//             const msg = response.error ?? response.message ?? "Failed to save report.";
//             setError(msg);
//             toast.error(msg);
//         } catch (err) {
//             console.error("Failed to save report", err);
//             const message = err instanceof Error ? err.message : "Failed to save report. Please try again.";
//             setError(message);
//             toast.error(message);
//         } finally {
//             setSaving(false);
...
//                                     if (fieldErrors.weekStart) setFieldErrors((f) => ({ ...f, weekStart: undefined }));
//                                 }}
//                             />
//                             {fieldErrors.weekStart && <p className="text-xs text-red-600">{fieldErrors.weekStart}</p>}
//                         </div>
//                         <div className="flex flex-col gap-1 flex-1">
//                             <Label>Week Ending</Label>
//                             <DatePicker label="Week ending date" value={weekEnd || null} disabled={true} />
//                         </div>
//                         <div className="flex flex-col gap-1 flex-1">
...
//                                 className={cn("min-w-[200px]! w-full", fieldErrors.project ? "border-red-500 focus-visible:ring-red-500" :"")}
//                             />
//                             {fieldErrors.project && <p className="text-xs text-red-600">{fieldErrors.project}</p>}
//                         </div>
//                     </div>

//                     <div className="flex flex-col gap-2">
//                         <div className="flex gap-5 justify-between items-center mb-1">
//                             <h1 className="text-base font-semibold">Tasks Completed</h1>
//                             <Button variant="outline" className="flex gap-1 items-center justify-center" onClick={addTask}>
//                                 <Plus />
//                                 Add Task
//                             </Button>
//                         </div>
//                         <div className={`border rounded-md overflow-x-auto ${fieldErrors.tasks ? "border-red-500" : ""}`}>
//                             <Table>
//                                 <TableHeader>
//                                     <TableRow>
...
//                                                     onChange={(e) =>
//                                                         setTasks((prev) =>
//                                                             prev.map((p, idx) => (idx === i ? { ...p, task_name: e.target.value } : p))
//                                                         )
//                                                     }
//                                                     placeholder="e.g. Build login page"
//                                                     className={cn("w-54 max-w-full", taskErrors[i]?.task_name ? "border-red-500 focus-visible:ring-red-500" : "")}
//                                                 />
//                                                 {taskErrors[i]?.task_name && <p className="text-xs text-red-600">{taskErrors[i].task_name}</p>}
//                                             </TableCell>
//                                             <TableCell>
//                                                 <Select
...
//                                                     onValueChange={(val) =>
//                                                         setTasks((prev) =>
//                                                             prev.map((p, idx) => (idx === i ? { ...p, priority: val as TaskPriority } : p))
//                                                         )
//                                                     }
//                                                 >
//                                                     <SelectTrigger className="w-36">
//                                                         <SelectValue placeholder="Medium">{t.priority ?? "Medium"}</SelectValue>
//                                                     </SelectTrigger>
...
//                                                             prev.map((p, idx) =>
//                                                                 idx === i
//                                                                     ? { ...p, planned_percentage: e.target.value === "" ? undefined : Number(e.target.value) }
//                                                                     : p
//                                                             )
//                                                         )
//                                                     }
//                                                     className={taskErrors[i]?.planned_percentage ? "border-red-500 focus-visible:ring-red-500" : ""}
//                                                 />
//                                                 {taskErrors[i]?.planned_percentage && <p className="text-xs text-red-600">{taskErrors[i].planned_percentage}</p>}
//                                             </TableCell>
//                                             <TableCell>
//                                                 <Input
...
//                                                             prev.map((p, idx) =>
//                                                                 idx === i
//                                                                     ? { ...p, actual_percentage: e.target.value === "" ? undefined : Number(e.target.value) }
//                                                                     : p
//                                                             )
//                                                         )
//                                                     }
//                                                     className={taskErrors[i]?.actual_percentage ? "border-red-500 focus-visible:ring-red-500" : ""}
//                                                 />
//                                                 {taskErrors[i]?.actual_percentage && <p className="text-xs text-red-600">{taskErrors[i].actual_percentage}</p>}
//                                             </TableCell>
//                                             <TableCell>
//                                                 <Select
...
//                                                     onValueChange={(val) =>
//                                                         setTasks((prev) =>
//                                                             prev.map((p, idx) => (idx === i ? { ...p, status: val as TaskStatus } : p))
//                                                         )
//                                                     }
//                                                 >
//                                                     <SelectTrigger className="w-36">
//                                                         <SelectValue placeholder="Not Started">{t.status ?? "Not Started"}</SelectValue>
//                                                     </SelectTrigger>
//                                                     <SelectContent position="popper">
//                                                         <SelectGroup>
...
//                                                             prev.map((p, idx) =>
//                                                                 idx === i
//                                                                     ? { ...p, time_planned_hours: e.target.value === "" ? undefined : Number(e.target.value) }
//                                                                     : p
//                                                             )
//                                                         )
//                                                     }
//                                                     className={taskErrors[i]?.time_planned_hours ? "border-red-500 focus-visible:ring-red-500" : ""}
//                                                 />
//                                                 {taskErrors[i]?.time_planned_hours && <p className="text-xs text-red-600">{taskErrors[i].time_planned_hours}</p>}
//                                             </TableCell>
//                                             <TableCell>
//                                                 <Input
...
//                                                             prev.map((p, idx) =>
//                                                                 idx === i
//                                                                     ? { ...p, time_spent_hours: e.target.value === "" ? undefined : Number(e.target.value) }
//                                                                     : p
//                                                             )
//                                                         )
//                                                     }
//                                                     className={taskErrors[i]?.time_spent_hours ? "border-red-500 focus-visible:ring-red-500" : ""}
//                                                 />
//                                                 {taskErrors[i]?.time_spent_hours && <p className="text-xs text-red-600">{taskErrors[i].time_spent_hours}</p>}
//                                             </TableCell>
//                                             <TableCell>
//                                                 <Input
...
//                                                     onChange={(e) =>
//                                                         setTasks((prev) =>
//                                                             prev.map((p, idx) => (idx === i ? { ...p, deliverable_output: e.target.value } : p))
//                                                         )
//                                                     }
...
//                             </Table>
//                         </div>
//                         {fieldErrors.tasks && <p className="text-xs text-red-600">{fieldErrors.tasks}</p>}
//                     </div>
...
//                                             value={b.text}
//                                             onChange={(e) =>
//                                                 setBlockers((prev) => prev.map((p, i) => (i === idx ? { ...p, text: e.target.value } : p)))
//                                             }
//                                             className="flex-1"
//                                         />
...
//                                                 name="blockerKey"
//                                                 checked={b.is_key}
//                                                 onChange={() => setBlockers((prev) => prev.map((p, i) => ({ ...p, is_key: i === idx })))}
//                                             />
//                                             <span className="text-sm">Key issue</span>
//                                         </div>
//                                         <button
//                                             type="button"
...
//                     <div className="space-y-3">
//                         <div className="flex items-center justify-between">
//                             <h2 className="text-sm font-semibold">Achievements / Highlights</h2>
...
//                                             value={a.text}
//                                             onChange={(e) =>
//                                                 setAchievements((prev) => prev.map((p, i) => (i === idx ? { ...p, text: e.target.value } : p)))
//                                             }
//                                             className="flex-1"
//                                         />
//                                         <div className="flex items-center gap-2">
...
//                                                 name="achievementKey"
//                                                 checked={a.is_key}
//                                                 onChange={() => setAchievements((prev) => prev.map((p, i) => ({ ...p, is_key: i === idx })))}
//                                             />
...
//                                 <ChevronDown
//                                     className={`h-4 w-4 transition-transform duration-200 ${hoursOpen ? "rotate-180" : "rotate-0"}`}
//                                 />
//                                 Hours by Task Type <span className="text-muted-foreground">(optional)</span>
//                             </button>
//                         </CollapsibleTrigger>
//                         <CollapsibleContent>
...
//                                     <Input value={hours.development} onChange={(e) => setHours((h) => ({ ...h, development: e.target.value }))} />
//                                 </div>
//                                 <div className="flex flex-col gap-1">
//                                     <Label>Testing</Label>
//                                     <Input value={hours.testing} onChange={(e) => setHours((h) => ({ ...h, testing: e.target.value }))} />
//                                 </div>
//                                 <div className="flex flex-col gap-1">
//                                     <Label>Meetings</Label>
//                                     <Input value={hours.meetings} onChange={(e) => setHours((h) => ({ ...h, meetings: e.target.value }))} />
//                                 </div>
//                                 <div className="flex flex-col gap-1">
//                                     <Label>Documentation</Label>
//                                     <Input value={hours.documentation} onChange={(e) => setHours((h) => ({ ...h, documentation: e.target.value }))} />
//                                 </div>
//                             </div>
//                         </CollapsibleContent>
//                     </Collapsible>
...
//                             placeholder="Any additional context, links to docs, PRs, etc."
//                         />
//                     </div>
