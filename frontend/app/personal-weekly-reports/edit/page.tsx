...
function EditReportContent() {
  ...
  useEffect(() => {
    if (!reportId) {
      setLoading(false);
      setError("A report ID is required to edit a report.");
      return;
    }
    ...
    const loadReport = async () => {
      ...
      try {
        const response = await getReportById(Number(reportId));
        if (!response.success || !response.data) {
          throw new Error(
            response.error ?? response.message ?? "Failed to load report.",
          );
        }

        const report = response.data;
        setWeekStart(report.week_start ?? "");
        setWeekEnd(report.week_end ?? computeWeekEnd(report.week_start ?? ""));
        ...
        setBlockers(
          (report.blockers ?? []).map((text) => ({
            text,
            is_key: text === report.key_blocker,
          })),
        );
        setAchievements(
          (report.achievements ?? []).map((text) => ({
            text,
            is_key: text === report.key_achievement,
          })),
        );
        setHours({
          development: String(
            report.time_breakdowns?.find(
              (item) => item.task_type === "Development",
            )?.hours_spent ?? "",
          ),
          testing: String(
            report.time_breakdowns?.find((item) => item.task_type === "Testing")
              ?.hours_spent ?? "",
          ),
          meetings: String(
            report.time_breakdowns?.find(
              (item) => item.task_type === "Meetings",
            )?.hours_spent ?? "",
          ),
          documentation: String(
            report.time_breakdowns?.find(
              (item) => item.task_type === "Documentation",
            )?.hours_spent ?? "",
          ),
        });
        setNotes(report.notes_or_links ?? "");
      } catch (err) {
        const message =
  ...
  const handleSave = async (
    ...
    if (!reportId) {
      setError("A report ID is required to save this report.");
      return;
    }
    ...
    try {
      const response =
        reportStatus === "Needs Correction" && status === "Submitted"
          ? await resubmitReport(Number(reportId), {
              version_number: reportVersion + 1,
              report_data: payload,
            })
          : await updateReport(Number(reportId), payload);
      if (response.success) {
        toast.success(
          status === "Draft"
  ...
  return (
    <div className="flex-1 w-full p-6 space-y-6 text-foreground">
      <div>
        ...
        <p className="text-muted-foreground">Edit details on the report</p>
      </div>
      ...
      {loading ? (
        ...
        <Card className="w-full !max-w-[900px] p-0 mx-auto gap-0">
          ...
          <div className="p-5 flex flex-col gap-5">
            ...
            {reportStatus === "Needs Correction" && reviewComment && (
              <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-100">
                ...
                <p className="mt-1 whitespace-pre-wrap">{reviewComment}</p>
              </div>
            )}
